...
function Event() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    city: "",
    eventId: null, // initialize eventId to 
  });
  ...
  const handleRegister = (eventId) => {
    setFormData((formData) => ({ ...formData, eventId: eventId })); // set eventId in the formData
    handleShow();
  };
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(
        `http://localhost:8000/events/${formData.eventId}/register/`, // use the eventId from formData
        formData
      )
      .then((response) => {
